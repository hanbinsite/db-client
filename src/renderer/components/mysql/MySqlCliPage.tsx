  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (command === 'USE' && params) {
        // 对于USE命令，我们直接使用它，但不需要返回结果集展示
        await window.electronAPI.executeQuery(poolId, sqlToExecute);
        // 更新内部数据库状态
        setCurrentDatabase(params);
        // 通知父组件更新数据库状态
        if (onDatabaseChange) {
          onDatabaseChange(params);
        }
        const resultLine: TerminalLine = {
          type: 'result',
          content: `数据库已切换到 ${params}`
        };
        ...
        // 执行SQL查询 - 确保使用当前选中的数据库上下文
        // 先检查是否需要先切换数据库再执行查询
          if (currentDatabase) {
            // 先执行USE命令切换到当前数据库上下文，并检查结果
            const useResult = await window.electronAPI.executeQuery(poolId, `USE \`${currentDatabase}\``);
            if (!useResult || !useResult.success) {
              const errorLine: TerminalLine = {
                type: 'error',
                content: `切换数据库失败: ${useResult?.error || '未知错误'}`
              };
              setTerminalOutput(prev => [...prev, errorLine]);
              return;
            }
          }
        const result = await window.electronAPI.executeQuery(poolId, sqlToExecute);
