...
const GaussDbConnectionPanel: ConnectionPanelComponent = ({
  form,
  connection,
  darkMode,
  onUrlChange,
  ...
  const handleFieldChange = () => {
    ...
    try {
      ...
      let url = `gaussdb://${values.username}`;
      if (values.password) {
        url += `:${values.password}`;
      }
      ...
      console.error('生成GaussDB连接URL失败:', error);
    }
  };
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col span={12}>
            <Form.Item
              ...
              wrapperCol={{ span: 18 }}
            >
              <Input placeholder="db-client" onChange={handleFieldChange} />
            </Form.Item>
          </Col>
        </Row>
      </div>
...
// 设置组件静态属性
GaussDbConnectionPanel.databaseType = 'gaussdb';
GaussDbConnectionPanel.label = 'GaussDB';
GaussDbConnectionPanel.defaultPort = 5432;

export default GaussDbConnectionPanel;