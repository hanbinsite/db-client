...
export interface GaussDBDatabaseTreeProps {
  treeData: TreeNode[];
...
const GaussDBDatabaseTree: React.FC<GaussDBDatabaseTreeProps> = ({
  treeData,
  expandedKeys,
  selectedKeys,
  onNodeSelect,
  ...
  console.log('GAUSSDB DATABASE TREE - 渲染GaussDB数据库树组件', {
  ...
  const handleSelect = (selectedKeys: React.Key[], info: any) => {
    console.log('GAUSSDB DATABASE TREE - 节点选择事件', { selectedKeys, info });
    ...
    try {
      ...
      if (info && info.node && info.node.props && info.node.props.dataRef) {
        ...
        console.log('GAUSSDB DATABASE TREE - 选择节点:', selectedNode.key, selectedNode.title);
        onNodeSelect(selectedNode);
      } else {
        console.log('GAUSSDB DATABASE TREE - 尝试从selectedKeys和treeData获取节点数据');
        ...
        if (selectedKeys && selectedKeys.length > 0) {
          ...
          for (const key of selectedKeys) {
            ...
            if (matchedNode) {
              console.log('GAUSSDB DATABASE TREE - 从treeData中查找到匹配节点:', key);
      ...
      console.error('GAUSSDB DATABASE TREE - 处理节点选择时发生错误:', error);
    }
  };

  // 处理节点展开/折叠
  const handleExpand = (expandedKeys: React.Key[], info: any) => {
    console.log('GAUSSDB DATABASE TREE - 处理节点展开/折叠:', expandedKeys, info);
    try {
      // 调用父组件传入的onExpand回调函数
      if (onExpand) {
        onExpand(expandedKeys);
        console.log('GAUSSDB DATABASE TREE - 节点展开/折叠事件已传递给父组件');
      }
    } catch (error) {
      console.error('GAUSSDB DATABASE TREE - 处理节点展开/折叠时发生错误:', error);
    }
  };

  // 处理节点双击事件
  const handleDoubleClick = (info: any) => {
    console.log('GAUSSDB DATABASE TREE - 节点双击事件:', info);
    try {
      // 从info中获取节点数据
      if (info && info.node && info.node.props && info.node.props.dataRef) {
        const clickedNode = info.node.props.dataRef;
        console.log('GAUSSDB DATABASE TREE - 双击节点:', clickedNode.key, clickedNode.title);
        // 调用父组件传入的onNodeDoubleClick回调函数
        if (onNodeDoubleClick) {
          onNodeDoubleClick(clickedNode);
          console.log('GAUSSDB DATABASE TREE - 节点双击事件已传递给父组件');
        }
      }
    } catch (error) {
      console.error('GAUSSDB DATABASE TREE - 处理节点双击时发生错误:', error);
    }
  };
        ...
        overlay={
          <Menu
            ...
            onClick={({ key }: { key: string }) => {
              if (onMenuSelect) {
                console.log('GaussDB右键菜单选择:', key, nodeData);
...
export default GaussDBDatabaseTree;